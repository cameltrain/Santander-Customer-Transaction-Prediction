...
```{r trian SVM Linear}
set.seed(3233)
modelSVMLinear <- train(factor(target)~., data = train, method = "svmLinear",
...
confusionMatrix(p_ClassSVMLinear, factor(test$target))
```
...
```{r ROC for SVMlinear}
ROC_SVMlinear<- roc(factor(test$target)~p_modelSVMLinear[,2], plot = TRUE, print.auc = TRUE, main="ROC for SVM withlinear kernel")
```
